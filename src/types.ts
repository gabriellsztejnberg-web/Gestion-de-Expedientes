...
export interface IncidenteDerrame {
  id: string;
  fecha: string; // YYYY-MM-DD
  mes: string; // YYYY-MM
  anio: number; // YYYY
...
export interface EmbarcacionPropia {
  id: string;
  nombre: string;
  matricula: string;
}
...
export interface BaseOperativa {
  ...
  materiales: string;
  cantidadBarreras?: number | string; // Suma total de barreras (Fluvial + Maritima + Puerto)
  ...
  embarcaciones?: number | string; // legacy o unificado
  embarcacionesDetalle?: EmbarcacionPropia[];
  metrosAbsorbentes?: number | string;
  otroEquipamiento?: string;
  observaciones?: string;
}
...
export interface ConvenioDerrames {
  ...
  empresaConvenidaId?: string;
  cantidadBarreras: number;
  cantidadAporta?: number;
