...
export const Users: React.FC = () => {
  ...
  // Flexibilidad total en el rol para evitar bloqueos
  const isJefe = currentUser.role?.toLowerCase() === 'jefe' || currentUser.role?.toLowerCase() === 'admin';

  if (!isJefe) {
    return (
      <div className="flex h-screen w-full items-center justify-center bg-background-light dark:bg-background-dark font-display p-6">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-xl shadow-xl border border-red-100 dark:border-red-900/20 text-center max-w-md">
          <span className="material-symbols-outlined text-red-500 text-5xl mb-4">gpp_maybe</span>
          <h2 className="text-slate-900 dark:text-white text-xl font-black uppercase mb-2">Acceso Restringido</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Solo personal con rango de JEFE puede gestionar la base de usuarios.</p>
        </div>
      </div>
    );
  }
